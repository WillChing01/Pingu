...
bool isValidInput(int argc, const char** argv) {
    if (argc < 5) {
        std::cout << "Error - expected to find at least 5 arguments" << std::endl;
        return false;
    }

    if (std::strcmp(argv[1], "out") || std::strcmp(argv[3], "in")) {
...
inline size_t writeDataToFile(const std::vector<Datum>& data, const std::filesystem::path& outputFilePath) {
    ...
    for (const Datum& datum : data) {
        if (datum.ply <= 12 || datum.totalPly - datum.ply < 6 || datum.timeLeft <= 10 || datum.totalTimeSpent <= 10) {
            continue;
        }

        outputFile << datum.fen << "," << datum.isDraw << "," << datum.isWin << "," << datum.ply << ","
                   << datum.totalPly << "," << datum.qSearch << "," << datum.inCheck << "," << datum.increment << ","
                   << datum.timeLeft << "," << datum.timeSpent << "," << datum.totalTimeSpent << "," << datum.startTime
...
int main(int argc, const char** argv) {
    populateMagicTables();
    processTimePgnCommand(argc, argv);
    delete[] hashTable;
    return 0;
}
