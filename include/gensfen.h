    ...
    for (size_t i = 0; i < requiredArgs.at(argc).size(); ++i) {
        ...
        if (!std::strcmp(type, "file")) {
            ...
            if (std::strcmp(argv[3 + 2 * i], "None") && !std::filesystem::exists(argv[3 + 2 * i])) {
                std::cout << "Error - the file '" << argv[3 + 2 * i] << "' does not exist in the current directory"
                          << std::endl;
                return false;
            }
        }
...
std::vector<std::string> parseBook(const std::string& bookFile) {
    ...
    std::regex fenRegex(
        R"((?![1-8PNBRQKpnbrqk\/]*\d{2,}[1-8PNBRQKpnbrqk\/]*)^([1-8PNBRQKpnbrqk]+\/){7}[1-8PNBRQKpnbrqk]+ [wb] (-|\bK?Q?k?q?) (-|[a-h][36])( [0-9]+ [0-9]+)?$)");
    std::smatch match;
...
bool playOpening(Search& s, int randomPly, const std::vector<std::string>& bookPositions) {
    ...
    for (int i = 0; i < randomPly; ++i) {
        ...
        // check for game over.
        if (!s.mainThread.b.moveBuffer.size()) {
            return false;
        }
        if (s.mainThread.isDrawByMaterial()) {
            return false;
        }
        if (s.mainThread.isDrawByRepetition()) {
            return false;
        }
        if (s.mainThread.isDrawByFifty()) {
            return false;
        }
...
void gensfenCommand(int argc, const char** argv) {
    ...
    while ((int)output.size() < positions) {
        ...
        // random playout.
        if (!playOpening(s, randomply, bookPositions)) {
            continue;
        }
        ...
        while (true) {
            ...
            if (!s.mainThread.b.moveBuffer.size()) {
                result = inCheck ? (s.mainThread.b.side ? 2 : 0) : 1;
                break;
            }
            if (s.mainThread.isDrawByMaterial()) {
                result = 1;
            ...
            if (std::abs(score) > evalbound) {
                result = score > evalbound ? 2 : 0;
                break;
            }
    ...
    std::string fileName =
        "gensfen_" + ENGINE_NAME_NO_SPACE + "_n" + std::to_string(output.size()) +
        (argc == 16 ? "_x" + std::to_string(nodes) : "_d" + std::to_string(mindepth) + "-" + std::to_string(maxdepth)) +
        "_r" + std::to_string(randomply) + "_m" + std::to_string(maxply) + "_b" + std::to_string(evalbound) + "_h" +
        std::to_string(hashSize) + "_" + bookFile + "_" + dateTime + ".txt";
