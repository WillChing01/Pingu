...
namespace validate
{
    ...
    inline bool isValidCastles(moveInfo &currentMove, bool inCheck, bool side, gameState &current, U64* pieces, U64* occupied)
    {
        //called by isValidMove, so currentMove is up-to-date.
    ...
    inline bool isValidMove(U32 chessMove, bool inCheck, bool side, gameState &current, U64* pieces, U64* occupied)
    {
        ...
        //check for pawn move.
        if ((currentMove.pieceType >> 1) == (_nPawns >> 1)) {return isValidPawnMove(currentMove, inCheck, current, pieces, occupied);}
        //check for castles.
