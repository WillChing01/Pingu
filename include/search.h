...
inline int alphaBeta(Board &b, int alpha, int beta, int depth, int ply, bool nullMoveAllowed)
{
    ...
    while (true)
    {
        ...
        if (score > bestScore)
        {
            ...
            {
                bool isQuiet =
                    movePicker.stage == KILLER_MOVES ||
                    movePicker.stage == QUIET_MOVES ||
                    (movePicker.stage == HASH_MOVE && movePicker.singleQuiets.contains(move));
                if (isQuiet)
                {
                    //update killers.
                    ...
                    if (depth >= 5)
                    {
                        if (movePicker.stage == QUIET_MOVES) {b.history.update(movePicker.singleQuiets, movePicker.scoredMoves, quietsPlayed - 1, move, depth);}
