...
inline int formatScore(int score) {
    return score > MATE_BOUND ? (MATE_SCORE - score + 1) / 2 : score < -MATE_BOUND ? (-MATE_SCORE - score) / 2 : score;
}
...
class Thread {
  public:
    ...
    bool isTimeUp() {
        if (std::chrono::duration<double, std::milli>(std::chrono::high_resolution_clock::now() - startTime).count() >
            searchTime) {
            return true;
        }
        return false;
    ...
    bool isDrawByMaterial() { return b.phase <= 1 && !(b.pieces[_nPawns] | b.pieces[_nPawns + 1]); }
    ...
    int search(int alpha, int beta, int depth, int ply, bool nullMoveAllowed) {
        ...
        // check for draw.
        if (isDrawByRepetition() || isDrawByMaterial() || isDrawByFifty()) {
            return 0;
        }
        ...
        if (hashInfo && getHashDepth(hashInfo) >= depth) {
            ...
            if (getHashExactFlag(hashInfo)) {
                return getHashEval(hashInfo, ply);
            }
            // Cut node.
        ...
        // null move pruning.
        if (hashInfo && getHashDepth(hashInfo) >= depth - nullMoveR - depth / 6 && !getHashBetaFlag(hashInfo) &&
            getHashEval(hashInfo, ply) < beta) {
        ...
        while (U32 move = movePicker.getNext()) {
            // late move pruning.
            if (canLateMovePrune && quietsPlayed > lateMovePruningMargins[depth - 1]) {
                break;
            }

            // futility pruning.
            if (canFutilityPrune && movesPlayed > 0 && movePicker.stage == QUIET_MOVES && !b.isCheckingMove(move)) {
                continue;
            }
            ...
            case BAD_CAPTURES:
                ...
                break;
            case QUIET_MOVES:
                if (canLateMoveReduce && movesPlayed > 0) {
                    reduction = lmrTable[depth - 1][movesPlayed];
                }
                break;
            }
            ...
            if (depth >= 2 && movesPlayed > 0) {
                if (reduction > 0) {
                    score = -search(-beta, -alpha, depth - 1 - reduction, ply + 1, true);
                } else {
                    score = alpha + 1;
                }
                    ...
                    if (!isSearchAborted) {
                        ...
                        if (isQuiet) {
                            ...
                            if (depth >= 5) {
                                ...
                                if (movePicker.stage == QUIET_MOVES) {
                                    if (hasPrevMove) {
                                        b.history.update(b.moveHistory.back(),
                                                         movePicker.singleQuiets,
                                                         b.moveCache[ply],
                                                         movePicker.moveIndex - 1,
                                                         move,
                                                         depth);
                                    } else {
                                        b.history.update(movePicker.singleQuiets,
                                                         b.moveCache[ply],
                                                         movePicker.moveIndex - 1,
                                                         move,
                                                         depth);
        ...
        if (!isSearchAborted) {
            ttSave(bHash, ply, depth, nodeBestMove, nodeBestScore, isExact, false);
        }
        return nodeBestScore;
    }
    ...
    void rootSearch(bool verbose) {
        ...
        if (b.moveBuffer.size() == 0) {
            bestScore = inCheck ? -MATE_SCORE : 0;
            isSearchFinished = true;
            return;
        }
        // if only one move return immediately.
        if (b.moveBuffer.size() == 1 && !analysisMode) {
            bestMove = b.moveBuffer[0];
            isSearchFinished = true;
            return;
        }
        ...
        for (int depth = 1; depth <= std::min(maxDepth, MAXDEPTH); ++depth) {
            ...
            if (depth > 1) {
                ...
                std::sort(rootMoves.begin(), rootMoves.end(), [](const auto& a, const auto& b) {
    ...
    void outputInfo(int depth, double totalTimeSpent) {
        ...
        std::cout << "info"
                  << " depth " << depth << " score " << (abs(bestScore) > MATE_BOUND ? "mate " : "cp ")
                  << formatScore(bestScore) << " time " << (U64)totalTimeSpent << " nodes " << globalNodeCount
                  << " nps " << nps << " pv";
