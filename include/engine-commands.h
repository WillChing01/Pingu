...
struct engineCommand {
    ...
    engineCommand(std::string commandName, std::string commandDesc, std::vector<std::string> commandUsage,
...
const std::map<std::string, engineCommand> CLI_COMMANDS = {
    {"bench", engineCommand("bench", "verify engine build and measure its speed", {"./Pingu bench"}, {})},
    {"gensfen",
     engineCommand("gensfen", "generate quiet positions for nnue training via fixed-depth self-play",
                   {
                       "./Pingu gensfen mindepth <n> maxdepth <n> positions <n> randomply <n> maxply <n> evalbound <n> "
                       "hash <n> book (None | <x>)",
                       "e.g. ./Pingu gensfen mindepth 8 maxdepth 12 positions 10000 randomply 4 maxply 200 evalbound "
                       "8192 hash 64 book None",
                   },
                   {
...
const std::map<std::string, engineCommand> UCI_COMMANDS = {
    {"uci", engineCommand("uci", "tell engine to use uci, id itself, and list options", {}, {})},
    {"isready", engineCommand("isready", "ping the engine to check that it is ready", {}, {})},
    {"setoption",
     engineCommand("setoption", "change the internal parameters of the engine",
                   {
    ...
    {"ucinewgame", engineCommand("ucinewgame", "tell the engine to prepare for a new game", {}, {})},
    {"position",
     engineCommand(
         "position", "set up the position on the internal board",
         {
    ...
    {"go",
     engineCommand("go", "start calculating on the current position",
                   {
    ...
    {"stop", engineCommand("stop", "stop calculating as soon as possible", {}, {})},
    {"eval", engineCommand("eval", "display static evaluation of current position", {}, {})},
    {"see",
     engineCommand("see", "perform static exchange evaluation in current position",
                   {
                       "see move <x>",
                       "e.g. see move e4d5",
                   },
    ...
    {"perft",
     engineCommand("perft", "count moves to given depth in current position",
                   {
                       "perft depth <n>",
                       "e.g. perft depth 5",
                   },
    ...
    {"test",
     engineCommand("test", "test engine features via perft-like search",
                   {
    ...
    {"display", engineCommand("display", "display the position of the internal board", {}, {})},
    {"quit", engineCommand("quit", "quit the program as soon as possible", {}, {})},
};
...
void outputHelpSummary(const std::map<std::string, engineCommand>& commands) {
    ...
    std::cout << "\nCommands:\n";
    for (const auto& [key, command] : commands) {
        std::cout << std::left << t << std::setw(maxLength + 4) << command.name << command.desc << "\n";
    }
}
...
void outputHelpVerbose(const std::string& key, const std::map<std::string, engineCommand>& commands) {
    ...
    if (command.options.size()) {
        ...
        std::cout << "\nOptions:\n";
        for (const auto& [option, desc] : command.options) {
            std::cout << std::left << t << std::setw(maxLength + 4) << option << desc << "\n";
        }
    }
    std::cout << std::endl;
}
