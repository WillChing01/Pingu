...
inline U32 getHashMove(const U64 info) { return info & BESTMOVEMASK; }

inline bool getHashExactFlag(const U64 info) { return info & EXACTFLAGMASK; }

inline bool getHashBetaFlag(const U64 info) { return info & BETAFLAGMASK; }

inline int getHashDepth(const U64 info) { return (info & DEPTHMASK) >> DEPTHSHIFT; }

inline int getHashEval(const U64 info, const int ply) {
    int eval = (info & EVALMASK) >> EVALSHIFT;
    if (!(info & EVALSIGNMASK)) {
        eval = -eval;
    }
...
inline int getHashAge(const U64 info) { return (info & AGEMASK) >> AGESHIFT; }
...
inline void ttSave(U64 zHash, int ply, int depth, U32 bestMove, int evaluation, bool isExact, bool isBeta) {
    ...
    U64 info = bestMove + (((U64)(depth) << DEPTHSHIFT) & DEPTHMASK) +
               (((U64)(abs(evaluation)) << EVALSHIFT) & EVALMASK) + ((U64)(evaluation > 0) << EVALSIGNSHIFT) +
               ((U64)(isExact) << EXACTFLAGSHIFT) + ((U64)(isBeta) << BETAFLAGSHIFT) +
...
inline U64 ttProbe(const U64 zHash) {
    ...
    // check deep table.
    if (hashTable[index].deepEntry.zHash == zHash) {
        return hashTable[index].deepEntry.info;
    }

    // check always table.
    if (hashTable[index].alwaysEntry.zHash == zHash) {
        return hashTable[index].alwaysEntry.info;
    }
