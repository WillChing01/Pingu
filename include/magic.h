...
const U64 rookMagics[64] = {
    36029071906791552ull,   1154047718063083552ull, 45040394859186176ull,   144141645376921604ull,
    216176080782889026ull,  180144534884189185ull,  74346811615084608ull,   648518904689213696ull,
    4611756388246364160ull, 6926580208451993668ull, 36046406385926656ull,   45669332186832952ull,
    650207780317429794ull,  2252083318228993ull,    576856585080016964ull,  289361773627181504ull,
    18028142547451936ull,   1152939131692778240ull, 1298162626984939552ull, 297259703111520256ull,
    74309471228133665ull,   4630298621074473480ull, 2305843627974459970ull, 289392289359339648ull,
    17636209467968ull,      4616260021561335808ull, 2594082186828185664ull, 1157451775982174720ull,
    4251433507554918528ull, 1126213473009856ull,    4513512417134744ull,    78813131320607491ull,
    4620711363919495168ull, 576480887111157776ull,  4611694815059378180ull, 594791819020605472ull,
    36033333059127296ull,   720611133360177288ull,  2378466963413270656ull, 144115326590124160ull,
    4936509241331566592ull, 147510480553394179ull,  9007474671888388ull,    345687014119571716ull,
    146652861265872928ull,  1152943494856771592ull, 300651627764844564ull,  20275011636301826ull,
    72348964895330306ull,   4612864695028678665ull, 5765808361598091290ull, 5297403042428093452ull,
    144150376743895572ull,  853977012899856ull,     36065081443762200ull,   146651230452601344ull,
    1207341085106434ull,    738590408161896577ull,  281612453413897ull,     4616752636795306498ull,
    4508006398034177ull,    562984380346466ull,     1179243505647617ull,    569839240356098ull,
};
...
const U64 bishopMagics[64] = {
    54052133425123392ull,   36150806904832ull,      71748519462928ull,      2308103713031716864ull,
    38852368850362368ull,   594555003046199296ull,  2306687850776625153ull, 4612259997900935200ull,
    1152922124256559116ull, 4611731102734747746ull, 90177437499392ull,      3458773998183127648ull,
    720647550402822148ull,  576496486720733186ull,  4793949099263008ull,    4573977016081408ull,
    3171695359388942892ull, 328762846533918848ull,  72621111128494085ull,   46179506221746178ull,
    145267484885327880ull,  35192967422982ull,      1876329959845272576ull, 2306442243789166592ull,
    613615724686475520ull,  4504462922493968ull,    2312601157836605968ull, 4828426157802194976ull,
    585758241435172868ull,  596727226646134912ull,  27059260349612548ull,   2594372590018232580ull,
    299101527098498ull,     18050222907597056ull,   1197436899967232ull,    2310390623668144640ull,
    290279659676160ull,     180219439216857352ull,  4611969710429570052ull, 1441222803956175872ull,
    1157715384967627264ull, 2305865583830566145ull, 18025535368073217ull,   405888120110717664ull,
    4900516729035163714ull, 18120236305749040ull,   2459038145157536010ull, 2379026778181087872ull,
    4611690434274795528ull, 288266111892324352ull,  1152941845639155712ull, 4683752984118427680ull,
    1235112207004598272ull, 1729400974816643072ull, 4630830749788939264ull, 864776924722106688ull,
    146384863816196612ull,  120490531624206848ull,  17314619649ull,         9024821506099200ull,
    280330567788ull,        3458817565846407296ull, 9011631795742848ull,    38280768765640720ull,
};
...
U64 getRandomU64FewBits() { return getRandomU64() & getRandomU64() & getRandomU64(); }
...
U64 getRookMagic(int square) {
    ...
    for (int _ = 0; _ < 10000000; _++) {
        ...
        if (__builtin_popcountll((rookMasks[square] * magic) & 0xFF00000000000000ULL) < 6) {
            continue;
        }
        ...
        for (int i = 0; i < (1 << rBits[square]); i++) {
            ...
            if (magicMoves[x] == 0ull) {
                magicMoves[x] = attackSets[i];
            } else if (magicMoves[x] != attackSets[i]) {
                good = false;
                break;
            }
        }
...
U64 getBishopMagic(int square) {
    ...
    for (int _ = 0; _ < 10000000; _++) {
        // get potential magicNum.
        U64 magic = getRandomU64FewBits();
        if (__builtin_popcountll((bishopMasks[square] * magic) & 0xFF00000000000000ULL) < 6) {
            continue;
        }
