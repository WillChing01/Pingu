...
class History {
  public:
    ...
    void increment(U32 prevMove, U32 move, int bonus) {
        ...
        delta = 32 * bonus -
                ((int)extendedScores[prevPieceType][prevFinishSquare][pieceType >> 1][finishSquare] * std::abs(bonus)) /
                    512;
    ...
    void update(const std::unordered_set<U32>& singles, const std::vector<std::pair<U32, int>>& quiets, U32 index,
                U32 cutMove, int depth) {
        int bonus = std::min(depth * depth, 400);
    ...
    void update(U32 prevMove, const std::unordered_set<U32>& singles, const std::vector<std::pair<U32, int>>& quiets,
                U32 index, U32 cutMove, int depth) {
        int bonus = std::min(depth * depth, 400);
