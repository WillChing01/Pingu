...
inline bool shouldCheckSEE(const U32 move) {
    ...
    return (capturedPieceType == 15) ||
           (pieceType >= _nQueens && seeValues[pieceType >> 1] > seeValues[capturedPieceType >> 1]);
}
  ...
  public:
    ...
    int evaluate(U32 chessMove) {
        ...
        gain[0] = (capturedPieceType != 15 ? seeValues[capturedPieceType >> 1] : 0) +
                  (attackingPieceType == ((chessMove & MOVEINFO_PIECETYPE_MASK) >> MOVEINFO_PIECETYPE_OFFSET)
                       ? 0
                       : seeValues[attackingPieceType >> 1] - seeValues[_nPawns >> 1]);
        ...
        if (chessMove & MOVEINFO_ENPASSANT_MASK) {
            occ ^= 1ull << (finishSquare - 8 + side * 16);
        }
        ...
        attackersBB |= magicRookAttacks(occ, finishSquare) &
                       (pieces[_nRooks] | pieces[_nRooks + 1] | pieces[_nQueens] | pieces[_nQueens + 1]);
        attackersBB |= magicBishopAttacks(occ, finishSquare) &
                       (pieces[_nBishops] | pieces[_nBishops + 1] | pieces[_nQueens] | pieces[_nQueens + 1]);
        ...
        do {
            ...
            if (attackingPieceType == (_nRooks >> 1) || attackingPieceType == (_nQueens >> 1) ||
                attackingPieceType == (_nKing >> 1)) {
                // rook-like xray.
                attackersBB |= magicRookAttacks(occ, finishSquare) &
                               (pieces[_nRooks] | pieces[_nRooks + 1] | pieces[_nQueens] | pieces[_nQueens + 1]) & occ;
            }
            if (attackingPieceType == (_nPawns >> 1) || attackingPieceType == (_nBishops >> 1) ||
                attackingPieceType == (_nQueens >> 1) || attackingPieceType == (_nKing >> 1)) {
                // bishop-like xray.
                attackersBB |= magicBishopAttacks(occ, finishSquare) &
                               (pieces[_nBishops] | pieces[_nBishops + 1] | pieces[_nQueens] | pieces[_nQueens + 1]) &
                               occ;
