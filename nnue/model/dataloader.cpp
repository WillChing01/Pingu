        ...
        for (size_t i = 0; i < 4; ++i) {
            ...
            while (x) {
                ...
                if (pieceType >= 2) {
                    ...
                    blackFeatures[totalFeatures] = (704 * (datum.kingPos[1] ^ 56)) +
                                                   64ull * (pieceType - 2ull * (pieceType & 1ull)) + (square ^ 56ull);
                    ++pieceCount[idx];
                    ++totalFeatures;
                }
            }
...
halfKaSparseBatch* getBatch(void* dataloader) { return static_cast<dataLoader*>(dataloader)->next(); }

void destructBatch(halfKaSparseBatch* batch) { delete batch; }
...
int main() {
    ...
    while (true) {
        ...
        if (batchCounter % 1000 == 0) {
            std::cout << batchCounter << std::endl;
        }
        ++batchCounter;
        delete batch;
    }
